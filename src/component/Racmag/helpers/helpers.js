...
export const returnFixNameRacmag = (name) => {
  ...
  const fixX8D = fix14C.replace("X8D ", "Honor X8D");
  const fixX8d = fixX8D.replace("X8d ", "Honor X8D");
  const fixLavender = fixX8d.replace("Lavander", "Lavender");
  ...
  replaceAwesome.indexOf("400 Pro") !== -1) ?
    replaceAwesome.replace("400", "Honor 400") : replaceAwesome
  const replaceSE = fixHonor400.replace("SE 8.7", "SE");
